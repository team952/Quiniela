...
/**
 * Actualiza nombre, zona horaria y módulos opcionales.
 * Recibe el `id` via `.bind()` desde el Client Component.
 * REGLA: no se puede ACTIVAR un módulo cuyo cierre ya pasó (sí se puede desactivar).
 */
export async function updateChampionship(
  ...
  if (fetchError || !current) return { error: 'Campeonato no encontrado o no tienes permisos.' }
  ...
  // Validar cierres fijos (solo si se intenta ACTIVAR un módulo inactivo)
